...
# Delete rg
if ($(az group exists -n $config.az_resource_group)) {
    Write-Host "Deleting resource group '$($config.az_resource_group)' and all resources in it..."
    az group delete --name $config.az_resource_group --yes | Out-String | ConvertFrom-Json | ConvertObjectToHashtable
    if ($LastExitCode -eq 0) {
        Write-Host "Resoruce group '$($config.az_resource_group)' deleted."
    }
} else {
